# 1) copy MS_IN -> MS_OUT
# 2) add obvious thermal noise via casatools.simulator.setnoise()
# 3) dirty image both (same params)
# 4) write diff image (after - before) and print stats
...
MS_OUT = "3c391_ctm_mosaic_10s_spw0.gaincal_corr.noise_obvious.ms"
...
IMG_BEFORE = "img_gaincal_before_noise"
IMG_AFTER  = "img_gaincal_after_noise"
IMG_DIFF   = "img_gaincal_after_minus_before_noise"
...
def simulator_obvious_noise_corrupt(msname: str):
    print(f"[INFO] Applying simulator noise corruption to {msname}")
    sm = simulator()
...
simulator_obvious_noise_corrupt(MS_OUT)
...
if np.any(finite):
    ...
    print("[WARN] diff image has no finite pixels (check selection / imaging outputs).")
