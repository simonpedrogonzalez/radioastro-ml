...
#2. trop corruption casatools.simulator.settrop
#3. imaging
#4. writes diff image
...
MS_OUT = "J1822_spw0.trop.corrupted.ms"

IMG_BEFORE_C = "img_trop_before_clean"
IMG_AFTER_C  = "img_trop_after_clean"
...
TROP_MODE = "individual"   # 'screen' or 'individual antennas' per docs
PWV_MM = 10000.0        # total PWV [mm] (bigger -> stronger effect)
DELTAPWV_FRAC = 1.9   # RMS PWV fluctuations as fraction of PWV (big for "obvious")
BETA = 1.0             # exponent of fractional brownian motion
WINDSPEED_MPS = 10000.0    # screen advection speed [m/s]
...
IMG_BEFORE = "img_trop_before"
IMG_AFTER  = "img_trop_after"
IMG_DIFF   = "img_trop_diff"
IMG_DIFF_C = "img_trop_diff_clean"
...
def make_clean(msname, outbase):
    ...
    tclean(vis=msname, imagename=outbase, niter=500, **TCLEAN_KW)
...
def sim_trop(msname: str):
    print(f"[INFO] Applying settrop to {msname}")
    sm = simulator()
    sm.openfromms(msname)
    sm.reset()
    sm.setseed(SEED)

    ttab = msname + ".Tcorrupt"
    rmtables(ttab)

    print("[INFO] calling settrop...")
    sm.settrop(
        table=ttab,
        mode=TROP_MODE,
        pwv=PWV_MM,
        ...
        simint=SIMINT_S,                   # IMPORTANT: let CASA choose
    )
    print("[INFO] settrop returned; table exists?", os.path.exists(ttab))
...
ttab = sim_trop(MS_OUT)

# from casatools import table
# tb = table()
# tb.open(ttab)
# print("TROP table columns:", tb.colnames())
...
IMG_FRAC_RES_BEFORE = "img_trop_before_fracres"
IMG_FRAC_RES_AFTER = "img_trop_after_fracres"
IMG_FRAC_RES_DIFF = "img_trop_fracres_diff"
